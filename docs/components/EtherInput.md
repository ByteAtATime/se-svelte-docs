...
![EtherInput Example](/img/components/EtherInput.gif)

## Import

```tsx
import { EtherInput } from "$lib/components/scaffold-eth";
```

## Usage

```tsx
const ethAmount = $state(""); // A string in order to store large values precisely

<EtherInput bind:value={ethAmount} />;
```
...
- **`value`**: The currently inputted value, in wei
